...
RegisterCommand('report', function(source, args, user)
    ...
    for k,v in pairs(reportcount) do
        if v.id == source then
            if v.gametimer + 120000 > GetGameTimer() and v.gametimer ~= 0 then
                TriggerClientEvent('esx:showAdvancedNotification', source, 'SUPPORT', '~b~'..GetPlayerName(source)..'', 'Vous devez patienter ~r~2 minute~s~ avant de faire de nouveau un ~r~report!', 'CHAR_BUGSTARS', 0)
                return
            else
    ...
    TriggerClientEvent('esx:showAdvancedNotification', source, 'REPORT', '~w~' ..GetPlayerName(source).. '', '~b~Votre Report a bien été envoyé ', 'CHAR_BUGSTARS', 0)
    ...
    for i = 1, #xPlayers, 1 do
        ...
        if xPlayer.getGroup() == "help" or xPlayer.getGroup() == "mod" or xPlayer.getGroup() == "admin" or xPlayer.getGroup() == "owner" or xPlayer.getGroup() == "superadmin" or xPlayer.getGroup() == "_dev" then
            TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'REPORT', 'Nouveau report de ~r~'..GetPlayerName(source)..' ~s~| ~b~'..source..'', 'Message: ~n~~b~'.. table.concat(args, " "), 'CHAR_BUGSTARS', 0)
...
AddEventHandler("OTEXO:onStaffJoin", function()
    ...
    for i = 1, #xPlayers, 1 do
        local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
        if xPlayer.getGroup() == "help" or xPlayer.getGroup() == "mod" or xPlayer.getGroup() == "admin" or xPlayer.getGroup() == "owner" or xPlayer.getGroup() == "superadmin" or xPlayer.getGroup() == "_dev" then
            TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'STAFF MODE', '', '~b~'..GetPlayerName(source)..'~s~ à ~b~activer~s~ son StaffMode ', 'CHAR_MP_FM_CONTACT', 0)
        end
    end
    if xPlayer.getGroup() ~= "user" then
...
AddEventHandler("OTEXO:onStaffLeave", function()
    ...
    for i = 1, #xPlayers, 1 do
        local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
        if xPlayer.getGroup() == "help" or xPlayer.getGroup() == "mod" or xPlayer.getGroup() == "admin" or xPlayer.getGroup() == "owner" or xPlayer.getGroup() == "superadmin" or xPlayer.getGroup() == "_dev" then
            TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, 'STAFF MODE', '', '~b~'..GetPlayerName(source).. '~s~ à ~b~désactiver~s~ son StaffMode ', 'CHAR_MP_FM_CONTACT', 0)
        end
    end
    if xPlayer.getGroup() ~= "user" then
...
AddEventHandler("OTEXO:ReportRegle", function(idt)
    ...
    if xPlayer.getGroup() ~= "user" then
        for i, v in pairs(allreport) do
            if i == idt then
                TriggerClientEvent('esx:showAdvancedNotification',  v.id, '[Assistance]', '', '~g~Votre report a été réglée !', 'CHAR_BUGSTARS', 0)
            end
        end
