...
local FastTravel = {
    { Name = "~w~Bat IAA~s~", Value = vector3(133.63, -628.14, 262.85) },
    { Name = "~w~Parking central~s~", Value = vector3(215.76, -810.8, 30.72) },
    { Name = "~w~Fourrière~s~", Value = vector3(409.16, -1625.47, 29.29) },
    { Name = "~w~Concessionaire~s~", Value = vector3(-67.6, -1091.21, 26.63) },
    { Name = "~w~Mécano~s~", Value = vector3(-211.44, -1323.68, 30.89) },
    { Name = "~w~Cayo Perico~s~", Value = vector3(4509.66, -4508.96, 3.01) },
}
...
function OTEXO.Helper:onToggleNoClip(toggle)
    if (toggle) then
        ...
        if (ESX.GetPlayerData()['group'] ~= "user") then
            ESX.ShowNotification("~o~Vous venez de désactiver le noclip")
    ...
    if (status) then
        ...
        CreateThread(function()
            while onStaffMode do
                Visual.Subtitle("~b~"..GetPlayerName(PlayerId()).."~s~ | Report actuels : ~b~" .. #OTEXO.GetReport , 999999999999999)
                Citizen.Wait(1000)
            end
        end)
...
local ped = {
    ...
    { Name = "Mexicain", Value = 'u_m_y_mani' },
    { Name = "Gi", Value = 'g_m_m_chigoon_01' },
    { Name = "Alejandro", Value = 'g_m_y_armgoon_02' },
    { Name = "Jack", Value = 'a_m_y_methhead_01' },
}
local pedIndex = 1
local ValuePed = 'u_m_y_zombie_01'
local NamePed = 'Zombie'
...
Citizen.CreateThread(function()
    while true do
        ...
        if (OTEXO.SelfPlayer.isStaffEnabled) then
            ...
            RageUI.Button('TP sur point', 'Permet de se ~b~téléporter~s~ sur un ~b~point~s~', { RightLabel = gang }, true, {
                onSelected = function()
                    plyPed = PlayerPedId()
        ...
        if (OTEXO.SelfPlayer.isStaffEnabled) then
            RageUI.IsVisible(utilsmenu, function()
                ...
                RageUI.Checkbox("Coordonnées", "Affiche les ~b~coordonnées", OTEXO.SelfPlayer.ShowCoords, { }, {
                    onChecked = function()
                ...
                RageUI.Checkbox("Delgun", 'Active le ~b~pistolet~s~ qui ~b~delete', OTEXO.SelfPlayer.isDelgunEnabled, { }, {
                    onChecked = function()
        ...
        if (OTEXO.SelfPlayer.isStaffEnabled) then
            RageUI.IsVisible(menugive, function()
                ...
                RageUI.Button("S'octroyer de ~g~l'argent ~s~en liquide", nil, { RightLabel = "→" }, true, {
                    onSelected = function()
                ...
                RageUI.Button("S'octroyer de ~g~l'argent ~s~en banque", nil, { RightLabel = "→" }, true, {
                    onSelected = function()
                        local amount = KeyboardInput('OTEXO_BOX_AMOUNT', "Veuillez entrer la somme", '', 8)
                ...
                RageUI.Button("S'octroyer de ~g~l'argent ~s~en sale", nil, { RightLabel = "→" }, true, {
                    onSelected = function()
                        local amount = KeyboardInput('OTEXO_BOX_AMOUNT', "Veuillez entrer la somme", '', 8)
        ...
        if (OTEXO.SelfPlayer.isStaffEnabled) then
            RageUI.IsVisible(vehiculemenu, function()
                ...
                RageUI.Button('~b~Réparation~s~ du véhicule', nil, { }, true, {
                    onSelected = function()
                        local plyVeh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
                        SetVehicleFixed(plyVeh)
                        SetVehicleDirtLevel(plyVeh, 0.0)
                ...
                RageUI.List('~b~Suppression~s~ des véhicules (Zone)', {
                    { Name = "1", Value = 1 },
                    { Name = "5", Value = 5 },
                    { Name = "10", Value = 10 },
                    { Name = "15", Value = 15 },
                ...
                RageUI.Button('~b~Changer~s~ la plaque', nil, {}, true, {
                    onSelected = function()
